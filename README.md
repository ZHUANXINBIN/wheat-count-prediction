...
Paper_code/
├── Model_training/
│   ├── DL/          # Deep-learning ablation pipeline (Section X.X)
│   └── ML/          # AutoGluon feature-based ML pipeline (Section X.X)
└── predict_process/ # End-to-end deployment pipeline (Section X.X)
```
...
7. `run_final_eval.py` — trains the top-3 architectures with 5-fold CV and
   evaluates once on the sealed test set (paper-reportable numbers).
8. `fix_model_selection.py` — post-hoc correction confirming model selection
   is based on development-pool validation R², not the sealed test set.
9. `sample_size_ablation.py` — sensitivity analysis on training-set size
...
2. `2_YOLO_detection/` — YOLOv11s-based wheat spike detection and
   pseudo-label annotation (includes the trained weights
...
The sealed test set (104 samples) and the full development pool (414 samples),
along with normalization statistics and train/test split indices, are archived
separately on Zenodo: `[DOI link to be inserted]`.
...
```
[Your paper citation, to be inserted after acceptance]
```
